...
Game::Game()
{
    ...
    this->window_height = desktop.size.y;
    this->window_width  = desktop.size.x;
...
void Game::Run()
{
    for (int i = 0; i < 250; i++) {
        ...
        shape.setPosition({static_cast<float>(window_width), static_cast<float>(window_height)}); // Testing purposes, starts all shapes in the center of screen.
...
void Game::HandleInput()
{
    ...
    if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
        ...
        shape.setPosition({static_cast<float>(mouseCoords.x), static_cast<float>(mouseCoords.y)});
...
void Game::Render()
{
    ...
    for (int i = 0; i < shapes.size(); i++) {
        ...
        auto theta = flock.getBoid(i).angle(flock.getBoid(i).velocity);
        shapes[i].setRotation(sf::degrees(theta));
